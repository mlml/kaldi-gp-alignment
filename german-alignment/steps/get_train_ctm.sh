...
if [ $stage -le 0 ]; then
  if [ -f $lang/phones/word_boundary.int ]; then
    $cmd JOB=1:$nj $dir/log/get_ctm_align.JOB.log \
      set -o pipefail '&&' linear-to-nbest "ark:gunzip -c $dir/ali.JOB.gz|" \
      "ark:utils/sym2int.pl --map-oov $oov -f 2- $lang/words.txt < $sdata/JOB/text |" \
      '' '' ark:- \| \
      lattice-align-words $lang/phones/word_boundary.int $model ark:- "ark:|gzip -c > $dir/aligned.JOB.gz"

    $cmd JOB=1:$nj $dir/log/get_ctm_nbest.JOB.log \
      nbest-to-ctm "ark:gunzip -c $dir/aligned.JOB.gz|" - \| \
      utils/int2sym.pl -f 5 $lang/words.txt \| \
      gzip -c '>' $dir/word_ctm.JOB.gz


  $cmd JOB=1:$nj $dir/log/get_ctm_align_phone.JOB.log \
    lattice-to-phone-lattice $model "ark:gunzip -c $dir/aligned.JOB.gz|" "ark:|gzip -c > $dir/aligned_phone.JOB.gz"

    $cmd JOB=1:$nj $dir/log/get_ctm_nbest.JOB.log \
      nbest-to-ctm "ark:gunzip -c $dir/aligned_phone.JOB.gz|" - \| \
      utils/int2sym.pl -f 5 $lang/phones.txt \| \
      gzip -c '>' $dir/phone_ctm.JOB.gz
  else
...
if [ $stage -le 1 ]; then
  if [ -f $data/segments ]; then
    ...
    for n in `seq $nj`; do gunzip -c $dir/word_ctm.$n.gz; done > $dir/word_ctm || exit 1;
    for n in `seq $nj`; do gunzip -c $dir/phone_ctm.$n.gz; done > $dir/phone_ctm || exit 1;
  fi
  rm $dir/word_ctm.*.gz
  rm $dir/phone_ctm.*.gz
  rm $dir/aligned.*.gz
  rm $dir/aligned_phone.*.gz
fi
